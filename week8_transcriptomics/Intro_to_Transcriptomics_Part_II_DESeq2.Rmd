...
rna_counts <- read.csv("/Users/aly/Desktop/Main/School_Files/S2/QBIO/qbio_490_alexzhang/analysis_data/brca_rna_count_data.csv")
rna_genes <- read.csv("/Users/aly/Desktop/Main/School_Files/S2/QBIO/qbio_490_alexzhang/analysis_data/brca_rna_gene_data.csv")
rna_clinical <- read.csv("/Users/aly/Desktop/Main/School_Files/S2/QBIO/qbio_490_alexzhang/analysis_data/brca_rna_clincial_data.csv")
...
# write code here
rna_clinical$age_category <- factor(rna_clinical$age_at_diagnosis, levels = c("old", "young"), labels = c("old", "young"))
...
```{r}
# write code here
summary(rna_clinical$stage_category)
#12 NA
is.na(rna_clinical$stage_category)
```
...
```{r}
na_mask <- !is.na(rna_clinical$stage_category)
...
rna_clinical <- rna_clinical[na_mask,]
  ...
  # think about where the mask goes in the df[rows, columns] notation
...
low_counts_mask <- ifelse(rna_counts > 10, "FALSE", "TRUE")
...
##-----------------------------------------------------------------------------------------------------------------------
#idk i couldnt load DESeq in time and its currently 1:03am :(
#my R also crashed midway thru and i lost all the work i did in the later questions pls be nice to me
##-----------------------------------------------------------------------------------------------------------------------
results <- data.frame(rna_genes$gene_names, rna_genes$gene_id, log2foldchange, pvalue, padj, -log10(padj))
...
colnames(results) <- c("Gene Names", "Gene ID", "log2foldchange", "pvalue", "padj", "-log10(fill in here") ## FIX
```
...
```{r}
up_reg_results <- data.frame(results$log2foldchange = c('indexes')
                             y = c('ordered'))

order_indexes = order(up_reg_results$log2foldchange)
...
```{r}
# write code here
up_reg_results <- up_reg_results > 1
```
...
```{r}
# write code here
down_reg_results <- up_reg_results < -1
```
...
```{r}
# write code here
colnames(up_reg_results) <- c("new row names for upreg")
colnames(down_reg_results) <- c("new row names for downreg")
```
...
```{r}
# write code here
i unfortunately cant do this bc i wasnt able to load DESeq
```
...
```{r}
# create plot here
EnhancedVolcano(par(mar=c(1,1,1,1))
                res,
                lab = rownames(res),
                x = 'log2FoldChange',
                y = 'pvalue',
                title = 'gene1 versus gene2',
```
...
EnhancedVolcano(res,
                ...
                labSize = 5.5,
                colAlpha = 1,
                legendLabels=c('gene1','gene2','p-value','log2foldchange'),
                legendPosition = 'left',
                legendLabSize = 15,
                legendIconSize = 4.0)
```
