...
no_mask <- !is.na(rna_se@colData$age_at_index)
rna_clinical <- rna_se@colData[no_mask, ]
```
...
#but i imported the data as a data frame so rna_clinical was already a data frame obj...?
```
...
```{r Exercise 2.6}
write.csv(rna_clinical, "/Users/aly/Desktop/Main/School_Files/S2/QBIO/qbio_490_alexzhang/analysis_data/brca_rna_clincial_data.csv", row.names = TRUE)
```
...
```{r Exercise 3.4}
write.csv(rna_genes, "/Users/aly/Desktop/Main/School_Files/S2/QBIO/qbio_490_alexzhang/analysis_data/brca_rna_gene_data.csv", row.names = FALSE)
```
...
```{r Exercise 4.4}
write.csv(rna_counts, "/Users/aly/Desktop/Main/School_Files/S2/QBIO/qbio_490_alexzhang/analysis_data/brca_rna_count_data.csv", row.names = TRUE)
```
...
```{r Exercise 5.4}
# write code here
rna_se$age_category <- ifelse(rna_clinical$age_at_index <= 58, "young", "old")
```
...
tissue_mask <- ifelse(colnames(rna_clinical$definition) == "Solid Tissue Normal", F, T)

rna_clinical$tissue_category <- rna_clinical$definition[,tissue_mask]
...
five_yr_death <- ifelse(rna_clinical$days_to_death == "NA", NA, ifelse(rna_clinical$days_to_death > 1826.25, T, F))

# use boolean data from days_to_last_followup when five_yr_death is NA

five_yr_death_and_followup <- ifelse(is.na(five_yr_death), ifelse(rna_clinical$days_to_birth == "NA", NA), five_yr_death)

# create your column from your finalized boolean vector (there should be only T/F, no NAs) 
rna_clinical$five_year_surv <- five_yr_death[five_yr_death_and_followup]
```
...
```{r Exercise 6.3}
# write code here
rna_genes$gene_name %in%("GATA3")
rna_genes$gene_name %in%("BAD")
...
GATA3_mask <- ifelse(rna_genes$gene_name == "GATA3", T, F) # create mask based on the gene name (your mask should be true when the name is 'geneA' and false for any other gene name)

BAD_mask <- ifelse(rna_genes$gene_name == "BAD", T, F)
```

We can use this mask for a few things. First, let's use it to grab the ensembl IDs of our genes.

```{r Exercise 6.5}
GATA3_ensembl <- ifelse(rna_genes$gene_id %in% row.names(result)) # think about which column we should apply out mask to
  
BAD_ensembl <- ifelse(rna_genes$gene_id %in% row.names(result))
```
...
```{r Exercise 6.6}
GATA3_subset <- subset(rna_genes, genes = 'GATA3')
GATA3_counts <- as.list(GATA3_subset[,-1])

BAD_subset <- subset(rna_genes, genes = 'BAD')
BAD_counts <- as.list(BAD_subset[,-1])
```
...
```{r Exercise 7.2}
geneAB_counts <- data.frame(GATA3_counts, BAD_counts) # create data frame with geneA_counts and geneB_counts
...
```{r Exercise 7.3}
par(mar=c(1,1,1,1))
boxplot(five_year_surv ~ GATA3_counts, # think about what data (gene counts or the 5 yr survival column) should be on the x vs y axes
        xlab = "GATA3 Count",
        ylab = "Survival")
dev.off()
```
...
```{r Exercise 7.4}
MKI67_subset <- subset(rna_genes, genes = 'MKI67')
MKI67_counts <- as.list(MKI67_subset[,-1])

PTGS2_counts <- subset(rna_genes, genes = 'PTGS2')
PTGS2_counts <- as.list(PTGS2_subset[,-1])
```
...
geneABCD_counts <- data.frame(GATA3_counts, BAD_counts, MKI67_counts, PTGS2_counts)
colnames(geneABCD_counts) <- c(GATA3_counts, BAD_counts, MKI67_counts, PTGS2_counts)
...
cols <- character(nrow(five_yr_surv)) 
cols[rna_clinical$COLUMN == TRUE] <- "blue" # fill in column and value for as many unique values are in your column (ex: 2 for age_category, "young"/"old")
cols[rna_clinical$COLUMN == FALSE] <- "red"
